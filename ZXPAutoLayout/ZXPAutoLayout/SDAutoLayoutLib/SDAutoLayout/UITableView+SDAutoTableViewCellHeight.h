...
// >>>>>>>>>>>>>> (单个cell)情景下调用以下方法(详细用法见demo5) >>>>>>>>>>>>>>

/**开启高度自适应，建议在tableview的数据源方法“- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section”中调用此方法*/
- (void)startAutoCellHeightWithCellClass:(Class)cellClass contentViewWidth:(CGFloat)contentViewWidth; //详细用法见demo5
...
// >>>>>>>>>>>>>> (多cell)情景下调用以下方法(详细用法见demo7) >>>>>>>>>>>>>>>>>
/**cellClassArray 为所有cell的类组成的数组*/
- (void)startAutoCellHeightWithCellClasses:(NSArray<Class> *)cellClassArray contentViewWidth:(CGFloat)contentViewWidth; //详细用法见demo7
/**返回计算出的cell高度;model为cell的数据模型实例;keyPath为cell的数据模型属性的属性名字符串（即kvc原理中的key）*/
