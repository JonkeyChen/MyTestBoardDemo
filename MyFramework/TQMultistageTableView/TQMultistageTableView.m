...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self){
        ...
        _tableView.separatorColor = HEX_RGB(0xefeff4);
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //2015-09-12 去除seperarColor 左侧默认的15单位线条
...
- (void)tableViewHeaderTouchUpInside:(UITapGestureRecognizer *)gesture{
    ...
    [self openOrCloseHeaderWithSection:(int)section];
}
...
- (void)insertRowWithSection:(int)section toIndexPaths:(NSMutableArray *)indexPaths{
    int insert = (int)[self dataSource_numberOfRowsInSection:section];
    if (insert != 0){
...
- (void)deleteRowWithSection:(int)section toIndexPaths:(NSMutableArray *)indexPaths{
    int delete = (int)[self dataSource_numberOfRowsInSection:section];;
    if (delete != 0){
...
- (void)openOrCloseHeaderWithSection:(int)section{
    ...
    if(self.selectIndexPath.section == section){
        [self deleteRowWithSection:(int)self.selectIndexPath.section toIndexPaths:deleteIndexPaths];
        self.selectIndexPath = [NSIndexPath indexPathForRow:-1 inSection:-1];
    } else {
